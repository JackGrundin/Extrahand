...
import { Text, TextInput, TouchableOpacity, StyleSheet, ScrollView, Alert, ActivityIndicator, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function RedigeraProfilScreen({ route, navigation }) {
  ...
  const [stadFel, setStadFel] = useState(false);
  const [laddar, setLaddar] = useState(false);

  // Uppdaterar stadsfältet och rensar felmarkering
  function hanteraStadInput(text) {
    setStad(text);
    if (stadFel) setStadFel(false);
  }

  async function spara() {
    // Om en stad är ifylld måste den vara vald från listan
    if (stad.trim() && !ärGiltigStad(stad)) {
      setStadFel(true);
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        {ärFöretag ? (
          ...
          <>
            ...
            <StadInput
              värde={stad}
              onÄndra={hanteraStadInput}
              placeholder="t.ex. Stockholm"
              fel={stadFel}
            />
            {stadFel && <Text style={styles.felText}>Välj en stad från listan</Text>}
          </>
        )}
